...
% ICTAI 2026 uses double-blind review. Add authors only to the accepted version.
...
message recovery is 100 percent in all 256 trials per operator. The complete detector,
rectifier, and neural decoder pipeline also recovers both payloads from all ten phone
photographs in a physical pilot. We report the capacity and imperceptibility tradeoff,
an architecture ablation, and a classical baseline.
\end{abstract}
...
manipulate modules, error-correction redundancy, segment choices, or color layers by
hand~\cite{lin2016qr}, \cite{koptyra2024segmentation},
\cite{tkachenko2016twolevel}, \cite{yang2018hiq}, \cite{chu2013halftone}. Several
preserve a public payload for a standard reader, but none of
the reviewed QR-specific methods trains an encoder and decoder jointly. Their evaluation
also differs from the screen-to-camera pipeline considered here.
...
secret without consuming the QR error-correction budget; the resulting symbols remain
valid under several readers and printed tests~\cite{koptyra2024segmentation}. The two-level
QR code replaces black modules with textured patterns recovered by correlation and
Reed-Solomon decoding~\cite{tkachenko2016twolevel}. HiQ superposes monochrome codes in
color and uses a trained classifier to separate them, so the composite is not a standard
monochrome QR~\cite{yang2018hiq}. Halftone QR binds the center of each submodule to the
decoded value while using the remaining area for a visible image~\cite{chu2013halftone}.
These methods establish that a QR symbol can carry private information, but their
embedding rules are designed manually. StegaQR instead learns the visual perturbation
and its decoder jointly against a degraded image channel.
...
capture. The closest neural neighbor is StegaStamp, which is physical but uses natural-image
covers. The closest QR neighbors are the classical redundancy, segmentation, and textured
module schemes, which preserve a public payload without learning the embedding channel.
Table~\ref{tab:related} summarizes these contrasts.

\begin{table*}[t]
\centering
\caption{Comparison with representative neural hiding and QR-specific methods. Public QR
indicates that a conventional monochrome reader still returns the public payload.}
\label{tab:related}
...
gradient clipping. Checkpoint selection maximizes the sum of raw full-codeword recovery
and bit accuracy under stochastic proxy distortion, with a small PSNR tie-break. This
avoids both a high-PSNR solution that loses bits and an unnecessarily strong perturbation.
...
\begin{table}[t]
\centering
\caption{Capacity scaling, cross-channel, trained and evaluated with proxy distortion.}
\label{tab:capacity}
\begin{tabular}{cccc}
\toprule
...
(Fig.~\ref{fig:capture}). This result validates the complete detection and decoding
pipeline; it does not claim that the neural decoder itself learns perspective invariance.
...
the physical pilot, while a standard reader recovers the public payload in every case.
The gains over a clean-trained model and a classical baseline are large, and the
imperceptibility and robustness ends of the system are characterized as a single tunable
frontier. Code and a tested package are prepared for release. Future work includes larger payloads, a broader physical
