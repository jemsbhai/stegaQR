...
## Framing decision: dual-regime (imperceptibility versus robustness frontier)
...
| Robust | ~18-22 dB | yes (color tint) | **robust data embedding / machine-covert color watermark** | physical capture (print/screen to camera) |
...
2. **Spatial bit-grid payload layout** - shown to be the enabling design (global
 broadcast à la HiDDeN/StegaStamp fails at L=100; grid to 100% in ~250 steps).
...
4. **Imperceptibility versus robustness versus capacity frontier**, plus ECC coupling that converts
 ~98-99% bit accuracy into ~100% message recovery.
5. **Real-world validation**: 100% hidden-message recovery from screen to phone-camera
...
3. **Method**
 ...
 - 3.5 Differentiable distortion layer + adaptive perceptual training (warmup to ramp)
...
4. **Experiments**
 ...
 - 4.6 **Physical screen to phone capture, 10/10** (fig_capture) - EXP-002
...
- ECC: Hamming(7,4) gives 100% message recovery; rep3 gives 99.5%.
...
- Imperceptible regime example: 54.9 dB / SSIM 1.000, public decode successful (fig_examples).
- Held-out true operators: 100% repetition-coded message recovery for 256 messages under each of 11 presets.
...
- Targeted novelty search completed 2026-07-16. Added Lin and Chen 2016 and Koptyra and Ogiela 2024. No learned method using a standard-decodable QR as the cover surfaced.
- MultiSpecQR remains omitted from the anonymous draft while it is under review. Revisit for the accepted version.
- Reference metadata and rendered citations verified.
