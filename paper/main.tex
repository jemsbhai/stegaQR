...
every condition, so the embedding never breaks the cover. Second, distortion training is
what produces robustness. Clean-trained models are near perfect on clean stego images but
...
hiders and train both under an identical budget. The comparison is decisive. At 100 bits
the spatial grid reaches 100 percent bit accuracy and 100 percent full-decode, clean and
distorted, while the broadcast layout stays at 51 to 53 percent bit accuracy, which is
chance, and never achieves a single full decode. The grid is therefore not an incremental
improvement but the component that makes the task learnable at this payload size.
...
full-decode. The learned, distortion-trained channel is what closes this gap.
...
The clearest limitation is the one the frontier makes explicit. The covert regime, where
...
spatial bit-grid that gives each bit a location, which is what lets a single shared
convolution learn the embedding at useful payload sizes where the global broadcast of prior
neural hiders stalls at chance. Three switchable modes trade fault isolation, capacity, and
structural guarantees, and a QR-anchored hybrid leaves the structural modules provably
untouched. Training end to end through a differentiable distortion layer, and coupling the
